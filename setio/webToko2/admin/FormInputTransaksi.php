...
<form action="ProsesInsertTransaksi.php" id="form1" method="POST"> 
  ...
  <div class="form-group row"> 
    ...
    <div class="col-sm-9"> <input type="date" name="tanggalTransaksi" class="form-control" id="tanggal-transaksi" value="<?php echo date('Y-m-j');?>" readonly> 
    </div> 
  </div> 
  <div class="form-group row"> 
    <label class="control-label col-sm-3">ID User(Kasir) / Nama User(Kasir)</label> <div class="col-sm-2"> 
      <input type="text" name="idUser" class="form-control" id="id-user" value="3" readonly> 
    </div> 
    <div class="col-sm-7"> 
      <input type="text" name="namaUser" class="form-control" id="nama-user" value="zakia" readonly> 
    </div> 
  </div> 
  <hr>
  <div class="row"> 
    ...
    <hr>
    <h2 align="center" style="padding-top: 15px;">Tabel transaksi</h2>
    <table class="table">
      ...
      <tbody>
        ...
        $result = $conn->query($sql); if ($result->num_rows > 0) { //Akan dijalankan jika recordnya ada 
          while ($row = $result->fetch_assoc()) { $totalHarga = $row['harga_barang'] * $row['jumlah_beli']; 
          ?>
          ...
          <tr>
            ...
            <td><?= $totalHarga; ?></td> 
            <td>
              <a onclick="return confirm('Anda yakin akan menghapus record ini?')" class="btn btn-danger" href="prosesDeleteTempTrans.php?id=<?= $row['id_barang'] ?>"> Delete </a> 
            </td> 
          </tr>
